...
#include "../make_file/make_file.h"                // 구동을 위해 일단 만듬. 필요여부에 따라 삭제 또는 편집작업 할 것
    ...
    for (i = 0; i < read_contents_length; i++)
    {
        ...
        if (started && !strncmp(&read_contents[i], ",", 1))
        {
            ...
            if (field_count % 5 == 4)
            {
                ...
                init_post_model_object(unique_id, title, writer, content, password);
            }
...
post_model **read_file_to_format(void)
{
    ...
    int created_file_descriptor = file_open(
        "/home/eddi/proj/SDC-Study-Team3/sungyonglee/created_file/format_post.txt", 
            O_RDONLY, 0644);
